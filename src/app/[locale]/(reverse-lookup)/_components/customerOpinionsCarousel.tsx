...
export const CustomerOpinionsCarousel: React.FC<CustomerOpinionsCarouselProps> = ({ className }) => {
      ...
      <div className="mt-6 select-none" ref={emblaRef}>
        <div className="flex">
          ...
          {(
            ...
            <div key={opinionKey} className="min-w-0 flex-[0_0_300px] pl-5">
              <Card
                className={`
              ...
              >
                <div className="flex flex-col gap-1">
                  {getOpinionText(opinionKey, 'title').trim() !== '' && (
                    <h4 className="text-lg font-semibold">{getOpinionText(opinionKey, 'title')}</h4>
                  )}
                  <StartRate className="size-[14px]" rating={Number.parseFloat(getOpinionText(opinionKey, 'rate'))} />
                  <p className="line-clamp-3 min-h-[72px]">{getOpinionText(opinionKey, 'description')}</p>
                  <p className="text-caption text-weak">
                    {getOpinionText(opinionKey, 'timeAgo')} • {getOpinionText(opinionKey, 'user')}
                  </p>
                </div>
              </Card>
            </div>
