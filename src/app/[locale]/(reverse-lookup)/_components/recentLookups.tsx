...
const RecentLookups: React.FC<RecentLookupsProps> = ({ originCountry = 'ES' }) => {
  ...
  return (
    <div className="grid w-full gap-4 pt-4 md:grid-cols-3 md:gap-9 md:pt-14">
      {items.length > 0 &&
        items.map((item) => (
          <Card key={`${item.phoneNumber}-${item.country}-${item.time}`} className="flex gap-4 p-4">
            <Flag size={40} countryCode={item.country as CountryCode} />
            <div className="flex flex-col gap-1">
              <h3 className="text-base leading-6 font-semibold text-strong lg:text-base">{item.title}</h3>
              <p className="text-sm text-weak">{t('result.description', { time: item.time })}</p>
