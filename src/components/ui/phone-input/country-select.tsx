...
const CountrySelect = ({ disabled, value, onChange, options }: CountrySelectProps) => {
  ...
  return (
    <Popover>
      ...
      <PopoverContent
        className="w-[330px] border-[hsla(var(--gray-transparent-500))] p-0"
        align="start"
        sideOffset={0}
        alignOffset={-4}
      >
        <Command>
          <CommandList>
            <ScrollArea className="h-72">
              <CommandInput
                className="border-[hsla(var(--gray-transparent-500))]"
              ...
              <CommandGroup>
                {options
                  .filter((x) => x.value)
                  .map((option) => (
                    <CommandItem className="gap-2 py-2" key={option.value} onSelect={() => handleSelect(option.value)}>
