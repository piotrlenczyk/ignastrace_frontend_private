...
const CORPORATE = new Set((args.corporate ?? "").split(",").map((s) => s.trim()).filter(Boolean));
...
const sourceOf = (dump) =>
  dump.remote === false ? "local" : CORPORATE.has(dump.collection) ? "corporate" : null;
...
for (const dump of varDumps) {
  ...
  if (!source) {
    excluded.push({ collection: dump.collection, variables: dump.rows.length, reason: "not part of the design system" });
    continue;
  }
...
const isTypography = (collection, name) =>
  /typograph|^font/i.test(collection) ||
  /^(font (size|weight|family)|line height|letter spacing)\//i.test(name);
...
const SECTIONS = ["primitives", "semantics", "shadows", "spacing", "radius", "sizing", "dimensions", "typography", "textStyles"];
...
if (styleDump) {
  ...
  for (const s of styleDump.rows) {
    ...
    const source =
      s.remote === false ? "local" : boundCollections.some((c) => CORPORATE.has(c)) ? "corporate" : null;
    if (!source) {
...
for (const [section, c] of Object.entries(counts)) {
  console.log(`  ${section.padEnd(11)} corporate ${String(c.corporate).padStart(4)}   local ${String(c.local).padStart(4)}`);
}
